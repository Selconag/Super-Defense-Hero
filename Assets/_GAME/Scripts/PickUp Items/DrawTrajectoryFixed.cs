...
public class DrawTrajectoryFixed : MonoBehaviour
{
    [SerializeField]
    ...
    public static DrawTrajectoryFixed Instance;
    ...
    //Straight Line Tracjectory
    public Vector3 UpdateTrajectory(Vector3 forceVector, Rigidbody rb, Vector3 startingPoint)
    {
        ...
        return _linePoints[_linePoints.Count - 1];
        //UpdateStraightTrajectory(rb.GetComponent<Transform>().position - _linePoints[_linePoints.Count - 1], rb, startingPoint);
    }



    //public void UpdateStraightTrajectory(Vector3 forceVector, Rigidbody rb, Vector3 startingPoint)
    //{
    //    Vector3 velocity = (forceVector / rb.mass) * Time.fixedDeltaTime;
    //    float FlightDuration = (2 * velocity.y) / Physics.gravity.y;
    //    float stepTime = FlightDuration / _lineSegmentCount;
